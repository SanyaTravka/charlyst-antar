(function () {
    ...
    for (const a of CALC.ATTRS) {
      ...
      box.appendChild(el(`
        <div class="row between" style="padding:0.3rem 0;border-bottom:1px solid var(--border);">
          ...
          <input class="field" type="number" min="1" max="30" data-action="attr-set" data-id="${esc(a)}" value="${char.attrs[a]}" style="width:4.5rem;">
        </div>
      `));
    }
    return box;
  ...
  function battleBlock(char, out) {
    ...
    box.appendChild(el(`
      <div class="row between" style="padding:0.3rem 0;">
        ...
        <input class="field" type="number" min="0" max="20" data-action="mastery-set" value="${char.masteryBonus}" style="width:4.5rem;">
      </div>
    `));
    return box;
  }
  ...
  function diceBlock(char) {
    ...
    if (tr && tr.inspirationDaily) {
      box.appendChild(el(`
        <div class="row" style="margin-top:0.5rem;">
          <button class="btn" data-action="reroll"${dis(char.inspiration <= 0 || !lastDice)}>Переброс (вдохновение: ${char.inspiration})</button>
  ...
  function handleClick(e) {
    ...
    else if (action === 'weapon-stop') { /* no-op */ }
  ...
  window.APP = { DATA, state, el, currentChar, mutate, save, toast, goto, render, esc, selectChar, deleteChar, exportChar, importChar, newChar, calcFull, tabSet };
})();
