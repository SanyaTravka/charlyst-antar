(function () {
  ...
  function renderSheet(app) {
    ...
    if (weaponEditIdx != null) {
      const em = weaponEditModal(weaponEditIdx);
      if (em) app.appendChild(em); else weaponEditIdx = null;
    }
  ...
  function renderRefbook(container) {
    ...
    if (q) {
      for (const id in DATA.allAbilities) {
        ...
        const body = [abilityMeta(a).join(' · '), a.desc || ''].filter(Boolean).join('\n');
  ...
  function abilityMeta(ab) {
    return [
      ab.tier ? 'Тир ' + ab.tier : '',
      ab.type === 'passive' ? 'пассивная' : 'активная',
  ...
  function abilityCard(char, id) {
    ...
    let rollRow = '';
    if (ab.type !== 'passive') {
      const ar = state.abRolls[id] || {};
      const curAttr = ar.attr || CALC.ATTRS[0];
      rollRow = `
        <div class="row" style="margin-top:0.45rem;flex-wrap:wrap;">
          <select class="field" data-action="ab-attr" data-id="${esc(id)}">
            ${CALC.ATTRS.map(a => `<option value="${esc(a)}"${a === curAttr ? ' selected' : ''}>${esc(a)}</option>`).join('')}
          </select>
    ...
    return el(`
      <div class="card" style="margin:0.5rem 0;">
        <div class="row between">
          <strong>${esc(ab.name)} <span class="muted small">${esc(abilityCostLabel(id))}</span></strong>
          ${btn}
        </div>
        <p class="small muted" style="margin:0.35rem 0 0;">${abilityMeta(ab).map(esc).join(' · ')}</p>
        ${rollRow}
  ...
  function abilityRoll(id) {
    const char = currentChar();
    const ab = DATA.allAbilities[id];
    if (!char || !ab || ab.type === 'passive') return;
    const ar = state.abRolls[id] || {};
  ...
  function abExtraSet(id, v) {
    const n = parseInt(v, 10);
    state.abRolls[id] = state.abRolls[id] || {};
    state.abRolls[id].extra = isNaN(n) ? 0 : n;
  }

  function abAttrSet(id, v) {
    state.abRolls[id] = state.abRolls[id] || {};
    state.abRolls[id].attr = String(v || '');
    render();
  }
  ...
  function customAbilitiesBlock(char, specId) {
    ...
    list.forEach(a => {
      box.appendChild(customAbilityCard(a, char.customAbilities.indexOf(a)));
    });
    box.appendChild(el(`
  ...
  function customAbilityCard(a, fullIdx, label) {
    return el(`
      <div class="card" style="margin:0.5rem 0;">
        <div class="row between">
          <strong>${esc(a.name)} <span class="muted small">Тир ${esc(a.tier)} · ${esc(a.type === 'passive' ? 'пассивная' : 'активная')} · ${esc(a.cost)}</span></strong>
          <button class="btn btn-danger" data-action="custom-del" data-id="${fullIdx}">${esc(label || 'Отдать')}</button>
        </div>
  ...
  function renderLearned(container, char) {
    const total = CALC.totalOS(char, DATA);
    container.appendChild(el(`
      <div class="row between" style="margin-bottom:0.75rem;">
        <h3 style="margin:0;">Изученные способности</h3>
        <span class="muted">ОС: ${char.spentOS} / ${total}</span>
      </div>
    `));
    let shown = 0;
    for (const sid of char.specializations) {
      const spec = DATA.specializations[sid];
      if (!spec) continue;
      const learned = char.abilities.filter(id => {
        const ab = DATA.allAbilities[id];
        return ab && ab.specId === sid;
      });
      const customs = char.customAbilities.filter(a => a.specId === sid);
      if (!learned.length && !customs.length) continue;
      const card = el(`<details class="card" style="margin-bottom:1rem;" open><summary style="cursor:pointer;font-weight:700;">${esc(spec.name)} <span class="muted small">${learned.length + customs.length} шт.</span></summary></details>`);
      for (const id of learned) card.appendChild(abilityCard(char, id));
      for (const a of customs) card.appendChild(customAbilityCard(a, char.customAbilities.indexOf(a)));
      container.appendChild(card);
      shown++;
    }
    if (!shown) {
  ...
  function renderSkills(container, char) {
    container.appendChild(el(`
      <div class="row between" style="margin-bottom:0.75rem;flex-wrap:wrap;gap:0.35rem;">
        ...
        <span class="muted small">d20 + мод характеристики · изученные + уровневый (${esc(char.masteryBonus || 0)}) · плюс свободный бонус</span>
      </div>
    `));
        ...
        body.appendChild(el(`
          <div class="row between" style="padding:0.3rem 0;border-bottom:1px solid var(--border);flex-wrap:wrap;gap:0.35rem;${trained ? 'border-left:4px solid var(--success);padding-left:0.45rem;' : ''}">
            ...
            <div class="row">
              ...
              <button class="btn" data-action="skill-trained" data-id="${esc(catId)}"${trained ? ' style="border:2px solid var(--success);font-weight:700;"' : ' style="opacity:0.7;"'}>${trained ? 'Изучен' : 'Не изучен'}</button>
              <input class="field" type="number" step="1" style="width:4rem;" title="Свободный бонус (можно отрицательный)" data-action="skill-bonus" data-id="${esc(catId)}" value="${state.skillBonus[catId] ? esc(state.skillBonus[catId]) : 0}">
  ...
  function skillRoll(catId) {
    ...
    const bonus = parseInt(state.skillBonus[catId], 10) || 0;
    const n = 1 + Math.floor(Math.random() * 20);
    lastDice = { desc: 'навык «' + sk.name + '»', roll: n, mod: mv + mb + bonus };
    const terms = [String(n), (mv >= 0 ? '+' : '') + mv];
      ...
      box.appendChild(el(`
        <div class="card" style="margin:0.5rem 0;">
          <div class="row between">
            ...
            <div class="row">
              <button class="btn" data-action="weapon-edit" data-id="${i}">Изменить</button>
  ...
  function weaponCustom(btn) {
    ...
    const data = readWeaponForm(modal);
    if (!data.name) { toast('Укажите название оружия', 'error'); return; }
    mutate(() => { currentChar().weapons.push(data); });
    weaponModalOpen = false;
    render();
  }

  function weaponEditModal(i) {
    const char = currentChar();
    const w = char && char.weapons[i];
    if (!w) return null;
    const b = weaponBase(w);
    const field = (action, placeholder, value, type) => `<input class="field"${type ? ` type="${type}"` : ''} placeholder="${esc(placeholder)}" data-action="${action}" value="${esc(value == null ? '' : value)}">`;
    return el(`
      <div class="wizard-overlay" data-action="weapon-editclose" style="position:fixed;inset:0;background:rgba(0,0,0,0.45);display:flex;align-items:center;justify-content:center;z-index:50;padding:1rem;">
        <div class="card wizard-modal" data-action="weapon-editstop" style="max-width:640px;width:100%;max-height:80vh;overflow:auto;">
          <div class="row between">
            <h3>Изменение оружия</h3>
            <button class="btn btn-danger" data-action="weapon-editclose">×</button>
          </div>
          <p class="small muted" style="margin:0.75rem 0 0.25rem;">Изменённое книжное оружие станет вашим экземпляром.</p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">
            ${field('wc-name', 'Название', b.name)}
            ${field('wc-kind', 'Тип', b.kind)}
            ${field('wc-speed', 'Атак/ход', b.speed != null ? b.speed : 1, 'number min="1" max="10"')}
            ${field('wc-atkbonus', 'Бонус к попаданию (+N)', b.atkBonus || 0, 'number step="1"')}
            ${field('wc-props', 'Свойства', b.props)}
            ${field('wc-reach', 'Досягаемость', b.reach)}
            ${field('wc-damage', 'Урон', b.damage)}
          </div>
          <div class="row" style="margin-top:0.75rem;">
            <button class="btn" data-action="weapon-editsave">Сохранить</button>
          </div>
        </div>
      </div>
    `);
  ...
  function weaponEditSave(btn) {
    const char = currentChar();
    if (!char || weaponEditIdx == null) return;
    const modal = btn.closest('.wizard-modal');
    if (!modal || !modal.querySelector) return;
    const data = readWeaponForm(modal);
    if (!data.name) { toast('Укажите название оружия', 'error'); return; }
    mutate(() => { char.weapons[weaponEditIdx] = data; });
    weaponEditIdx = null;
    render();
  }
  ...
  function diceBlock(char) {
    ...
    box.appendChild(el(`
      <div class="row" style="margin-top:0.5rem;">
        ...
        <input class="field" type="number" step="1" style="width:4rem;" title="Свободный бонус (можно отрицательный)" data-action="check-bonus" value="${state.checkBonus ? esc(state.checkBonus) : 0}">
        <button class="btn" data-action="check-roll">Проверка</button>
        <span class="small muted">d20 + мод + свой бонус</span>
      </div>
    `));
    return box;
  }
  ...
  function checkRoll(btn) {
    ...
    const mv = CALC.mods(char, DATA)[attr] || 0;
    const bonus = parseInt(state.checkBonus, 10) || 0;
    const n = 1 + Math.floor(Math.random() * 20);
    lastDice = { desc: 'проверка «' + attr + '»', roll: n, mod: mv + bonus };
    const terms = [String(n), (mv >= 0 ? '+' : '') + mv];
    if (bonus !== 0) terms.push((bonus > 0 ? '+' : '') + bonus);
    showRoll(
  ...
  function handleClick(e) {
    ...
    else if (action === 'weapon-edit') { weaponEditIdx = parseInt(id, 10); render(); }
    else if (action === 'weapon-editclose') { weaponEditIdx = null; render(); }
    else if (action === 'weapon-editstop') {}
    else if (action === 'weapon-editsave') weaponEditSave(t);
  ...
  function handleInput(e) {
    ...
    else if (action === 'check-bonus') {
      const n = parseInt(t.value, 10);
      state.checkBonus = isNaN(n) ? 0 : n;
    }
    else if (action === 'skill-bonus') {
      const n = parseInt(t.value, 10);
      state.skillBonus[t.getAttribute('data-id')] = isNaN(n) ? 0 : n;
    }
