...
**Goal:** Один самодостаточный HTML-файл `antar-sheet.html` — интерактивный чарлист для НРИ «Антар» 0.99 с мастером создания, авто-расчётами, боевым трекером, левел-апом и сохранением в localStorage + JSON.
...
- Источник правил: `docs/antar-rules-0.99.txt` (извлечён из `Антар-правила 0.99.docx`; в Task 1 извлекался 0.98 — оба файла лежат в `docs/`, транскрипция идёт из 0.99, см. Task 4b)
...
- ОС за уровни (правила): 1:3, 2:2, 3:2, 4:2, 5:3, 6:3, 7:2, 8:2, 9:2, 10:3, 11:3, 12:2, 13:2, 14:2, 15:3; 16+ — «ждём редакций» (вручную в левел-апе)
...
### Task 4b: Адаптация данных к редакции правил 0.99

> Заменяет поведение, заданное в Task 2 (п. 226, 274, 385, 424), Task 3 (п. 677–688) и Task 4 (п. 720, 756–757, 775): редакция 0.99 продлила таблицу ОС до 15-го уровня, удалила из правил специализацию «Очарование», у «Воинской магии» появилась полная ветка (дерево данных придёт в Task 5), изменён урон арбалетов. Задачи 1–4 уже закоммичены по 0.98 — патчим код и тесты.

**Files:**
- Modify: `src/data-core.js`, `src/calc.js`, `test/data-core.test.js`, `test/calc.test.js`

**Consumes:** `docs/antar-rules-0.99.txt` (специализации 123–138, ОС 152–170, арбалеты 2904–2919)

- [ ] **Step 1: Написать падающие тесты**

В `test/data-core.test.js` заменить тест `osByLevel 1..10 и tiers` (стр. 126–127) на:
```js
...
```
Добавить тест специализаций 0.99 (13 шт., нет «Очарования», «Призыв» пустая, у «Воинской магии» метки `empty` нет):
```js
...
```
В `test/calc.test.js`: фикстуру `osByLevel` (стр. 21) продлить до `{ 1: 3, 2: 2, 3: 2, 4: 2, 5: 3, 6: 3, 7: 2, 8: 2, 9: 2, 10: 3, 11: 3, 12: 2, 13: 2, 14: 2, 15: 3 }`; тест `totalOS` (стр. 103–104): для level=11 — `27` = `3+2+2+2+3+3+2+2+2+3+3`; добавить level=15 → `36`; level=16 → `36` (16+ ОС не даёт — вводится вручную в левел-апе).

- [ ] **Step 2: Прогнать — FAIL**

Run: `npm test`
Expected: FAIL (osByLevel deepEqual, totalOS 24→27, специализации 14→13).

- [ ] **Step 3: Реализовать**

`src/calc.js` (стр. 29): `Math.min(level, 10)` → `Math.min(level, 15)` (16+ вклад из osByLevel = 0).

`src/data-core.js`:
- `osByLevel` — добавить ключи `11: 3, 12: 2, 13: 2, 14: 2, 15: 3`
- `specializations` (стр. 192–205): удалить запись `enchantment` (школы «Очарование» в 0.99 нет); у `battleMagic` убрать `empty: true` (ветка в 0.99 есть, дерево данных — Task 5); `conjuration` остаётся `empty: true`
- `weapons.lightCrossbow.damage` (стр. 224): `'10+2d10 колющего'` (0.99, строка 2909)
- `weapons.heavyCrossbow.damage` (стр. 225): `'15+2d12 колющего'` (0.99, строка 2918; speed 1 + «перезарядка действием следующего хода» остаётся как есть — пометка о перезарядке уже в ledger L9, рендер UI не трогаем)
...
Run: `node build.js; npm test`
```bash
git add src/data-core.js src/calc.js test/data-core.test.js test/calc.test.js antar-sheet.html docs/antar-rules-0.99.txt
git commit -m "adapt data to rules 0.99 (OS 1..15, 13 specializations, crossbow damage)"
```

---

### Task 5: data-magic.js — 8 магических школ (все заклинания)
...
**Interfaces:**
- Consumes: `docs/antar-rules-0.99.txt` (Проявление 509–709, Восстановление 710–888, Преобразование 889–1115, Иллюзия 1116–1329, Ограждение 1330–1564, Антимагия 1565–1717, Проклятия 1718–1872, Воинская магия 1883–2048). «ПРИЗЫВ» (1873–1882) — только текст-блок без способностей: conjuration остаётся пустой (дерево рендерится пустым из `DATA_CORE.specializations[id].empty`), записи не создаём.
...
- battleMagic (Воинская магия, дерево 0.99) T1: Свечение, Стихийная закалка, Укрепление тетивы, Заряженный снаряд, Силовой щит; T2: Перехват, Связь с оружием, Скорая кара, Истребление; T3: Послушное оружие, Карающий удар, Дуговой удар; T4: Стронгхолд, Изгоняющий взмах (в правилах заголовок «ТИР :» — это ТИР 4)

Примечание: в 0.99 у заклинаний тиров 1–2 изменён урон (напр. Пламенные руки nd4→nd6, Снаряды 1d4+1→1d6+1, Едкое облако 2d6→4d6, Магический камень 5→7) и у части 4-го тира — затраты (напр. Энергетические клинки 10→15, Восстановление сил 8→12, Регенерация 8→10, Невидимость 10→15, Контрзаклинание 5→6, Сфера поглощения 10→12) — транскрибировать строго из `docs/antar-rules-0.99.txt`, не из памяти и не из 0.98.
...
git commit -m "feat: spell trees for 8 magic schools per rules 0.99"
```

---

### Task 6: data-physical.js — 4 физические ветки (все готовые)
...
**Interfaces:**
- Consumes: `docs/antar-rules-0.99.txt` (Воинское искусство 2049–2270, Ловкость 2271–2416, Живучесть 2417–2584, Сила 2585–2768)
...
- martial (2049–2270) T1: Адепт оружия, Атака с финтом, Отвлекающий удар, Атака с выпадом, Оборонительная стойка, Легкие удары, Подсечка, Пристрелка, Наступательная стойка, Малая концентрация; T2: Знаток оружия, Отталкивающий удар, Опрокидывающий удар, Обезоруживание, Жестокий удар, Контроль, Останавливающий удар, Контратака, Провокация, Натиск, Рокировка, Кровоточащий удар, Передышка, Намётанный глаз, Раскалывающий удар; T3: Эксперт оружия, Подавление, Калечащий удар, Вихрь, Нырок, Рефлексы, Залп (новый в 0.99, стр. 2249: «Затрата: 3 запаса сил»); T4: Мастер оружия, Рассечение
- dexterity (2271–2416) T1: Увертливость, Легкий в ногах, Уж, Спринт, Активное уклонение, Быстрые руки; T2: Мифическая ловкость, Неприметное движение, Прорехи, Гений защиты, Ловля снарядов; T3: Поступь ветра, Широко шагая, Адреналин, Резкий, Предрекая смерть, Покоритель неба, Юркий; T4: Бег пера, Налегке, Великая скорость (и остальные по тексту до 2416)
- vitality (2417–2584) T1: Горячка, Живительный покой, Самолечение, Граница жизни, Перераспределение энергии, Ускоренное лечение; T2: Стальные легкие, Выработанный иммунитет, Мифическая живучесть, Крепость тела, Закалка, Затягивание ран; T3: Драконья кожа, Кровожадность, Регенерация, Адамантиновые кости, Непрошибаемое строение, Природная живучесть, Контроль тела, Второе дыхание; T4: Несгибаемый, Великое превозмогание, Болевой порог, Нисходящий, Заторможенное старение
- strength (2585–2768) T1: Цепкая хватка, Руки чемпиона, Крепок в ногах, Натиск, Демонстрация силы, Атлет; T2: Тяжелая рука, Разрушение, Мифический атлетизм, Пугающее присутствие, Сокрушающие удары; T3: Сила великана, Пружина, Исключительная мощь, Титан, Бронебойность, Жнец, Изверг, Совершенный удар, На равных; T4: Преодоление, Таран, Лязг, Двужильный, Дрожь земли, Неудержимый
...
Примечание: в 0.99 изменены затраты/урон части способностей (напр. у Рассечение затрата 15 запаса сил, стр. 2266) — транскрибировать строго по тексту `docs/antar-rules-0.99.txt`, не из 0.98 и не по памяти.
...
Список 13 специализаций (0.99; «Очарование» удалено из правил — карточки с пометкой «телесная/приобретённая»; пустая одна — conjuration «Призыв», пометка «ждём редакций»); выбор: ровно 4 (или 5 при `draft.traitId === 't19'`); гном: телесные (strength/dexterity/vitality) не выбираются (disabled при `raceId === 'gnome'`); «Далее» при нужном количестве.
...
3. ОС: `gainOS = osByLevel[newLevel] || 0` + правила черт: t1 «Приспосабливаемый»: если newLevel % 3 === 0 → +1 (в сумму totalOS автоматически — здесь показать); t12 «Тупой»: −1; для 16+ уровня — input «ОС за уровень (ручн.)»: показывается, если newLevel > 15, значение добавляется
